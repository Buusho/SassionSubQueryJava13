...
--Найти всех пациентов, которые лечатся в отделении 'Surgical’
select *
from patients p
where doctor_id in (select d.id
                    from doctors d
                    where department_id in (select d.id from departments d where d.department_name = 'Surgical'));
--Получить список всех отделений, где работает врач с именем 'Miller’
select *
from departments
where id in (select department_id from doctors where doctors.first_name = 'Miller');
--Найти всех пациентов, у которых врач имеет стаж более 10 лет
select *
from patients p
where p.doctor_id in (select d.id from doctors d where d.experience > 10);
--Получить список всех врачей и количество пациентов, которых они обслуживают
select d.id, d.first_name, count(p.id) as patient_count
from doctors d
         left join patients p on d.id = p.doctor_id
group by d.id, d.first_name
order by d.id;
--Получить список всех пациентов, которые не обслуживаются ни у одного врача
select p.first_name
from patients p
where p.doctor_id is null;
--Получить список всех врачей, которые не обслуживают ни одного пациента
select *
from doctors d
         inner join departments de on d.id = de.hospital_id
         inner join patients p on d.id = p.doctor_id is null;
select *
from doctors d
where d.department_id in (select de.id
                          from departments de
                          where de.hospital_id in (select p.id from patients p where p.doctor_id is null));
--Получить список всех врачей, которые лечат пациентов старше 60 лет
--Найти всех пациентов, которые лечатся у врача с именем 'Conrado' и фамилией 'Placide’
select *
from patients p
where p.doctor_id in (select d.id from doctors d where d.first_name = 'Conrado' and d.last_name = 'Placide');
--Получить список всех врачей, работающих в отделении 'Intensive' и обслуживающих больше 3 пациентов
select *
from doctors d
         inner join departments dp on d.department_id = dp.id
         inner join (select doctor_id, count(*) from patients p group by doctor_id having count(*) > 1) p
                    on d.id = p.doctor_id
where dp.department_name = 'Intensive';

--Получить список всех отделений, в которых пациенты лечатся у врачей с опытом менее 5 лет
select *
from departments dp
         inner join doctors d on d.department_id = dp.id
         inner join (select doctor_id, patients.first_name from patients group by doctor_id, patients.first_name) p
                    on d.id = p.doctor_id
where d.experience < 5;
--Найти всех пациентов, у которых врач имеет специализацию 'DERMOTOLOGIST’
select *
from patients p
where p.doctor_id in (select d.id from doctors d where d.specialization = 'DERMOTOLOGIST');
select *
from patients p
         inner join doctors d on p.doctor_id = d.id
where d.specialization = 'DERMOTOLOGIST';
--Получить количество врачей для каждой специализации
select specialization, count(*)
from doctors
group by specialization;
--Найти всех пациентов, обслуживаемых врачами с наименее распространенной специализацией
select *
from patients p
         inner join doctors d on p.doctor_id = p.id
where d.specialization = (select specialization from doctors group by specialization order by count(*) asc limit 1);
--Найти всех пациентов, обслуживаемых врачами с специализацией ‘CARDIOLOGIST’
select *
from patients p
where p.doctor_id in (select d.id from doctors d where d.specialization = 'CARDIOLOGIST');
select *
from patients p
         inner join doctors d on p.doctor_id = d.id
where d.specialization = 'CARDIOLOGIST';
--Получить список всех врачей, работающих в отделении 'PSYCHIATRIST' и обслуживающих больше 3 пациентов
select *
from doctors d
         inner join departments dp on d.department_id = dp.id
         inner join (select doctor_id, count(*) from patients group by doctor_id having count(*) > 1) p on d.id = p.doctor_id
where dp.department_name = 'Psychical';